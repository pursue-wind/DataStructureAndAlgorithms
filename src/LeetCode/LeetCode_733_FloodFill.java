 ...
 * 像素点，接着再记录这四个方向上符合条件的像素点与他们对应四个方向上像素值与初始坐标相同的相连
 * 像素点，……，重复该过程。将所有有记录的像素点的颜色值改为新的颜色值。
 * ===========================================================================================
 * 最后返回经过上色渲染后的图像。
 * ===========================================================================================
 * 示例 1:
...
public class LeetCode_733_FloodFill {
    ...
    private void dfs(int[][] image, int x, int y, int curColor, int newColor) {
        ...
        if (image[x][y] == curColor) {
            image[x][y] = newColor;
            for (int i = 0; i < 4; i++) {
                int curX = i != 3 ? x + i - 1 : x + i - 3;
                int curY = i == 0 ? y + i : y + i - 2;
                if (curX > -1 && curY > -1 && curX < image.length && curY < image[0].length) {
...
//        int[][] image = new int[][]{{0, 0, 0}, {0, 0, 0}};
        image = new LeetCode_733_FloodFill().floodFill(image, 1, 1, 2);
